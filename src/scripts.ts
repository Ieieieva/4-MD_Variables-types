...
function filterCourses(products: Products[]) {
  return products.filter(product => product.type === 'Course')
}

function getTitles(products: Products[]) {
...
const courses = filterCourses(products) //: Products[]
const titles = getTitles(products) //:String[]
...
const currentAccount = <Account1> fetchAccount(4)
